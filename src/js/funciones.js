...
export function initCRUD({
    ...
    async function cargar() {
        try {
            const { [entity]: items } = await apiFetch(
                `/app01_DGCM/${entity}/obtener${capitalize(entity)}`
            );
            lista = items;
    ...
    Form.addEventListener("submit", async e => {
        ...
        try {
            const data = await apiFetch(
                `/app01_DGCM/${entity}/guardar${capitalize(entity)}`,
    ...
    btnModificar.addEventListener("click", async e => {
        ...
        try {
            const data = await apiFetch(
                `/app01_DGCM/${entity}/modificar${capitalize(entity)}`,
                { method: "POST", body: formData }
            );
            await mostrarAlerta("success", "Éxito", data.mensaje);
            resetForm();
    ...
    tabla.on("click", ".btn-editar", async ev => {
        ...
        try {
            const { [entity.slice(0, -1)]: item } = await apiFetch(
                `/app01_DGCM/${entity}/buscar${capitalize(entity.slice(0, -1))}?` +
    ...
    tabla.on("click", ".btn-eliminar", async ev => {
        ...
        try {
            ...
            const data = await apiFetch(
                `/app01_DGCM/${entity}/eliminar${capitalize(entity)}`,
                { method: "POST", body: fd }
            );
            await mostrarAlerta("success", "Éxito", data.mensaje);
            await cargar();
